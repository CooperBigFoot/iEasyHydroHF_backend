...
class TestTimeseriesQueryManager:
    ...
    def test_local_datetime_equal_utc_datetime(
        self,
        organization,
        water_level_manual,
        water_level_manual_other,
        ...
        datetime_local = dt.datetime(2020,1,1,0,0,0,tzinfo=ZoneInfo('Asia/Tashkent'))
        datetime_utc = dt.datetime(2019,12,31,19,0,0,tzinfo=ZoneInfo('UTC'))

        metric_utc = HydrologicalMetric(
        ...
        metric_utc.save()
        metric_local = HydrologicalMetric(
        ...
        query_manager = TimeseriesQueryManager(
            ...
            organization_uuid=organization.uuid,
            filter_dict={
                "timestamp_local__gte": dt.datetime(2019,1,1,0,0,0,tzinfo=ZoneInfo('UTC')).isoformat(),
            },
        )
        ...
        print(results.query)
        assert results.count() == 2
        assert list(results) == [metric_local, metric_utc]
