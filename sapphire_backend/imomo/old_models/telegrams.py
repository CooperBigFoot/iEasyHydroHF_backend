...
class Telegram(ImomoBase):
    telegram_id = Column("TelegramID", Integer, Sequence("telegramid_telegram_seq"), primary_key=True)
    ...
    status = Column(
        "Status",
        Enum(*[status.name for status in list(TelegramStatus)], name="telegram_statuses"),
    ...
    def to_jsonizable(self):
        ...
        if self.updated_on is not None:
            json_telegram["updatedOn"] = calendar.timegm(self.updated_on.utctimetuple())
