...
class ReportingTemplate(ImomoBase):
    ...
    def to_jsonizable(self, public_url=None, exclude=None):
        ...
        json_ = super(ReportingTemplate, self).to_jsonizable(exclude)
    ...
    def __repr__(self):
        return '<ReportingTemplate: {type} on source {source_id}>'.format(
            type=self.type, source_id=self.source_id
        )
...
class ReportSchedule(ImomoBase):
    ...
    def to_jsonizable(self, exclude=None):
        ...
        json_ = super(ReportSchedule, self).to_jsonizable(exclude)
    ...
    def get_next_issue_date(self, date=None):
        ...
        if self.type == TemplateTypeEnum.decadal_bulletin.name:
            ...
            if day_of_period <= issue_date_offset:
                start, end, issue_date = timeseries.get_current_period_details(
                    date, issue_date_offset, 'decade'
                )
            else:
                start, end, issue_date = timeseries.get_next_period_details(
                    date, issue_date_offset, 'decade'
                )
    ...
    def __repr__(self):
        return '<ReportSchedule: {type} on source {source_id}>'.format(
            type=self.type, source_id=self.source_id
        )
