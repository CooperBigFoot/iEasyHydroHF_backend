...
class _SurrogatePKMixin:
    ...
    def id(cls):
        ...
        """
        return Column(Integer, Sequence("%s_id_seq" % cls.__name__.lower()), primary_key=True)


class _ClassTableNameMixin:
            ...
            if column_name in json_object:
                ...
                if isinstance(column.type, UTCDateTime):
                    if type(create_params[column.name]) is float:
                        create_params[column.name] = datetime.datetime.utcfromtimestamp(
                            create_params[column.name]
                        ).replace(tzinfo=datetime.UTC)
                    elif isinstance(create_params[column.name], datetime.datetime):
                        create_params[column.name] = create_params[column.name].replace(tzinfo=datetime.UTC)
                elif isinstance(column.type, DateTime):
                    create_params[column.name] = datetime.datetime.utcfromtimestamp(create_params[column.name])
