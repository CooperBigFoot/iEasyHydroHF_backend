...
class User(ImomoBase):
    ...
    registered_on = Column(UTCDateTime, nullable=False, default=lambda: datetime.datetime.now(datetime.UTC))

    source = relationship(Source, innerjoin=True, lazy="joined")
    invitation = relationship("UserInvitation", cascade="all,delete")

    role = Column(
        Enum(*[role.name for role in list(UserRoleEnum)], name="roles"), nullable=False, default=UserRoleEnum.user
    )
    ...
    def validate_registered_on(self, key, registered_on):
        ...
        assert registered_on <= datetime.datetime.now(datetime.UTC)
    ...
    def to_jsonizable(self, exclude=None):
        ...
        return super().to_jsonizable(exclude)
    ...
    def __repr__(self):
        try:
            if self.source_id is not None:
                ...
                organization_name = ""
            return f"<User: {self.username} ({self.role} @ {organization_name})>"
        except Exception:
...
class UserInvitation(ImomoBase):
    ...
    def to_jsonizable(self, exclude=None):
        return_json = super().to_jsonizable(exclude)
