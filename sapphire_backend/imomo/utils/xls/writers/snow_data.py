...
class SnowDataWriter(xlsxwriter.Workbook):
    ...
    def write(self, site_snow_data):
        ...
        worksheet.write(0, 0, "Min elevation", self._column_header_format)

        worksheet.write(1, 0, "Max elevation", self._column_header_format)

        row = 2
        for index, values in enumerate(site_snow_data["resources"]):
            worksheet.write(0, index + 1, values["min_elev"], self._column_header_format)

            worksheet.write(1, index + 1, values["max_elev"], self._column_header_format)

        if all_keys:
            for single_date in self.date_range(start_date, end_date):
                date_key = single_date.strftime("%Y-%m-%d")
                worksheet.write(row, 0, date_key, self._column_header_format)
                for index, values in enumerate(site_snow_data["resources"]):
                    worksheet.write(row, index + 1, values["snow_data"].get(date_key, ""), self._number_format)
