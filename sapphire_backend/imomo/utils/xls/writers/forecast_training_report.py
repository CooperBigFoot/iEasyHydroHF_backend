...
class ForecastTrainingReportDataWriter(xlsxwriter.Workbook):
    ...
    def __init__(self, filename=None, options=None):
        options = options or {}
        super(ForecastTrainingReportDataWriter, self).__init__(filename, options)
    ...
    def write(self, real_data, forecasted_data, frequency):
        ...
        for column, header in enumerate(self._get_headers(frequency)):
            ...
            worksheet.write(
                0, column, header, self._column_header_format
            )
        ...
        for x, (index, values) in enumerate(all_values.iteritems()):
            ...
            row = x + 1
            column = 0
            for column, period_value in enumerate(
                    self._get_period_values(index, frequency)
            ):
                worksheet.write(
                    row,
                    column,
