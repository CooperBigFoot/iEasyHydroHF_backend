...
class HistoricDataWriter(xlsxwriter.Workbook):
    ...
    def write(self, site_historic_data):
        for data_type, historic_data in site_historic_data.iteritems():
            ...
            worksheet.write(0, 0, to_unicode(_("Year")), self._column_header_format)
            ...
            if all_years_with_data:
                ...
                for index, year in enumerate(range(min_year, max_year + 1)):
                    ...
                    self._write_row(worksheet, index + 1, year_data, str(year), hydrology_format)

        self.close()

    def _write_row(self, worksheet, row_index, row_data, row_header, hydrology_format=False):
        worksheet.write(row_index, 0, row_header, self._row_header_format)
