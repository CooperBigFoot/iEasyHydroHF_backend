...
class BulkDataWriter(xlsxwriter.Workbook):
    ...
    def __init__(self, filename=None, options=None):
        if options is None:
            options = {}
        super(BulkDataWriter, self).__init__(filename, options)
    ...
    def write(self, data_values):
        for site, site_data in data_values.iteritems():
            ...
            variable_data = sorted(
                variables,
                key=lambda x: x.get('sort', x.get('variable').variable_code)
            )
            ...
            worksheet.write(
                1, 0, to_unicode(_('Date / Unit')), self.add_format(self.header_format))
