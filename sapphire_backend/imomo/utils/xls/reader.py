...
class XLSReader(object):
    ...
    def get_sheet_by_index(self, index, raise_exception):
        try:
            ...
            if raise_exception:
                raise errors.XLSReaderError(
                    'Sheet with index {} not found.'.format(index)
                )
    ...
    def get_sheet_by_name(self, sheet_name, raise_exception):
        try:
            ...
            if raise_exception:
                raise errors.XLSReaderError(
                    'Sheet with name {} not found.'.format(sheet_name)
                )
    ...
    def parse_decade_data(sheet):
        ...
        for row in xrange(1, sheet.nrows):
            ...
            if year < 1900 or year > 2100:
                raise errors.XLSReaderError('Invalid year {}'.format(year))
