...
class Tag(object):
    ...
    TAG_START = '{{'
    TAG_END = '}}'
    tag_re = r"{tag_start}(.*?){tag_end}".format(
        tag_start=TAG_START,
        tag_end=TAG_END,
    )
    ...
    def full_tag(self, special=None):
        if special is not None:
            ...
            special_extension = '{}{}'.format(special, self.SPLIT)
        else:
            special_extension = ''
        return '{tag_start}{ext}{tag}{tag_end}'.format(
            tag_start=self.TAG_START,
            tag_end=self.TAG_END,
            tag=self.tag,
            ext=special_extension,
        )
    ...
    def full_header_tag(self):
        return '{tag_start}{tag}{tag_end}'.format(
            tag_start=self.TAG_START,
            tag_end=self.TAG_END,
            tag=self.tag,
        )
...
class ReportGenerator(object):
    ...
    tags = (
        ...
        Tag(
            ...
            _('Discharge decade norm on the current day with ignored data newer '
        ...
        if tag_type == Tag.HEADER_TAG:
            if not tag_obj.header:
                raise errors.ValidationError(
                    _('Tag {} is not valid HEADER tag.').format(tag))

            if self.header_tag is not None:
                raise errors.ValidationError(
                    _('Can\'t have multiple HEADER tags.'))

        if tag_type == Tag.DATA_TAG:
            if not tag_obj.data:
                raise errors.ValidationError(
                    _('Tag {} is not valid DATA tag.').format(tag))
    ...
    def validate_header_and_data_tags(self):
        ...
        if not self.data_tags:
            raise errors.ValidationError(
                _('At least one DATA tag is required.'))
        ...
        for cell in self.data_cells:
            ...
            if data_row != data_row_:
                raise errors.ValidationError(
                    _('All DATA tags should be in the same row.'))

        if data_row - header_row != 1:
            raise errors.ValidationError(
                _('HEADER tag should be exactly 1 row above DATA tags.'))
    ...
    def move_cell(
        ...
        if cell_to_move.data_type == 'f':
            ...
            moved_cell.value = Translator(
                cell_to_move.value,
                adr.format(
                    get_column_letter(source_address[1]), source_address[0])
            ).translate_formula(
                adr.format(
                    get_column_letter(dest_col), dest_row)
            )
        ...
        if move_merged:
            for range_ in worksheet.merged_cell_ranges:
                ...
                if source_cell == worksheet[start_cell_]:
                    ...
                    worksheet.merged_cell_ranges.append('{}{}:{}{}'.format(
                        cell_to_move.column,
                        cell_to_move.row,
                        end_cell.column,
                        end_cell.row,
    ...
    def generate_report(
            self, filename, session, sites, date=None, language='en'
    ):
        ...
        for group_header, sites in grouped_sites.iteritems():
            ...
            self.replace_value(
                dest_cell,
                group_header,
                self.header_tag.full_tag(Tag.HEADER_TAG)
            )
            current_row += 1
        ...
        for tag, cells in self.general_tags.iteritems():
            ...
            tag_value = tag.get_value(
                date=date, session=session, context=context, language=language)
