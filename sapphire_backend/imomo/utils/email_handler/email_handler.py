...
app = Celery(
    "hydromet",
    backend=f"redis://{secrets.REDIS_HOST}:{secrets.REDIS_PORT}",
    broker=f"redis://{secrets.REDIS_HOST}:{secrets.REDIS_PORT}",
)
...
class EmailServer:
    base_template_vars = {
        ...
        "footer": "<br>iMoMo team<br>hs@hydrosolutions.ch<br>" "Lindenbachstrasse 11, 8006 Zürich, Switzerland",
        "header": "iEasyHydro",
    }
    ...
    def send_register_email(cls, created_user, password):
        ...
        template_vars = {
            "intro_message": _(
                ...
                "Hydrometeorology application. Your username is:"
            ).format(full_name=to_str(created_user.get("full_name", ""))),
            "password_message": _("Here is also a generated password for your account:"),
            "message_final": _(
                " We highly recommend you to change your " "generated password immediately after login."
            ),
        ...
        send_email(_("Welcome to iEasyHydro"), formatted_template, send_to)
    ...
    def send_reset_password(cls, full_name, token, send_to):
        template_vars = {
            ...
            ).format(full_name=to_str(full_name)),
        ...
        formatted_template = template.render(**template_vars)
        send_email(_("iEasyHydro password"), formatted_template, send_to)
    ...
    @classmethod
    def send_super_admin_warning_email(cls, source_name, message, admins, super_admins):
        admin_emails_html = ", ".join([f'<a href="mailto:{admin.email}">{admin.full_name}</a>' for admin in admins])
        template_vars = {"organization_name": source_name, "message": message, "admin_emails": admin_emails_html}
        template_vars.update(cls.base_template_vars)
        cls.encode_utf8(template_vars)
        template = cls.load_template(template_name="expiration_warning_admin")
        formatted_template = template.render(**template_vars)
        send_email_("iEasyHydro subscription expiration", formatted_template, [admin.email for admin in super_admins])

    @classmethod
    def send_user_warning_email(cls, source_name, message, admins):
        template_vars = {
            "intro_message": _("Subscription to the iEasyHydro " "application for <b>{org}</b> organization:").format(
                org=to_str(source_name)
            ),
            "details_message": _(
                "For more details, please contact " '<a href="mailto:hs@hydrosolutions.ch"> iMoMo team</a>'
            ),
        ...
        formatted_template = template.render(**template_vars)
        send_email_(_("iEasyHydro subscription expiration"), formatted_template, [admin.email for admin in admins])
    ...
    def send_invitation_email(cls, user_invitation, admin):
        template_vars = {
            "message_1": _(
                ...
                "of <b>{organization}</b> organization."
            ).format(
                full_name=to_str(user_invitation.full_name), organization=to_str(user_invitation.source.organization)
            ),
            "message_2": _(
                "For more details, please contact the organization admin, "
                '<a href="mailto:{admin_email}">{admin_full_name}</a>.'
            ).format(
                admin_email=to_str(admin.email),
                admin_full_name=to_str(admin.full_name),
            ),
            "register_href": f"{secrets.FRONTEND_URL}/invitations/?invitation_uuid={user_invitation.uuid}",
    ...
    def send_report_email(
        ...
        template_vars = {
            ...
            "message": _("New <b>{report_type}</b> report has been generated:").format(report_type=_(report_type)),
        ...
        formatted_template = template.render(**template_vars)
        send_email_(_("iEasyHydro {} report").format(_(report_type)), formatted_template, subscriptions)

    @classmethod
    def send_retraining_email(cls, organization_name, admins):
        template_vars = {
            "message": _("All forecasting models related to the <b>{org}</b> " "organization was retrained.").format(
