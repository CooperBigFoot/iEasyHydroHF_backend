...
class PostgreSQLTool(cherrypy.Tool):
    ...
    def _setup(self):
        ...
        cherrypy.request.hooks.attach("after_error_response", self.rollback_transaction, priority=80)
...
class DBContextManager:
    ...
    def __enter__(self):
        ...
        logger.info(f"DB connection created: {id(self)}")
        return self.session

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.engine.dispose()
        logger.info(f"DB connection closed: {id(self)}")
