...
from imomo import ROOT_PATH, models
...
def _controlled_vocabulary_path(cv_name):
    return os.path.abspath(os.path.join(ROOT_PATH, os.path.pardir, DATA_DIR, ODM_DIR, "%s.xml" % cv_name.lower()))


UNITS_PATH = os.path.abspath(os.path.join(ROOT_PATH, os.path.pardir, DATA_DIR, ODM_DIR, "units.xml"))
SPATIAL_REFERENCES_PATH = os.path.abspath(
    os.path.join(ROOT_PATH, os.path.pardir, DATA_DIR, ODM_DIR, "spatial-references.xml")
)
...
def load_controlled_vocabulary(imomo_init, cv_name, tries=0):
    ...
    try:
        ...
        for record in records:
            imomo_init.session.add(cv_model(term=record[0].text, definition=record[1].text))
        imomo_init.commit()
    except:
        if tries > MAX_TRIES:
            raise
...
def load_spatial_references(imomo_init, tries=0):
    try:
        ...
        for record in records:
            ...
            max_id = max(int(record[0].text), max_id)
        imomo_init.commit()
        imomo_init.engine.execute(RESTART_SEQ_SQL % ("spatialreference_id_seq", max_id + 1))
    except:
        if tries > MAX_TRIES:
            raise
...
def default_odm_objects(imomo_init):
    ...
    topic_category = (
        imomo_init.session.query(models.TopicCategoryCV).filter(models.TopicCategoryCV.term == "Unknown").one()
    )
    iso_metadata_default = models.ISOMetadata(id=0, topic_category_id=topic_category.id)
