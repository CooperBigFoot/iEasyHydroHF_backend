...
def get_or_create_auto_station_by_code(station_code: str) -> HydrologicalStation | None:
    """
    Get or create automatical hydro station in case there is already a manual station with the
    same station_code. Currently it is expected that for all automatic stations there is already
    a corresponding manual station.
    """
    hydro_station_auto_obj = None
    try:
        hydro_station_auto_obj = HydrologicalStation.objects.get(
            station_code=station_code, station_type=HydrologicalStation.StationType.AUTOMATIC
        )
